"""Dataset + samplers for the FoNE arm.

Parallel to dataset.py but emits the extra fields the FoNE model needs:
  - input_ids, target_tokens: standard shifted-pair token sequences
  - num_values: per-position numeric values (0 except at NUM positions)
  - sign_target: per-sample result-c sign class id
  - digit_target: per-sample result-c 6 base-10 digits
  - is_nan: per-sample bool mask (digit-loss don't-care)

Re-uses the existing classified pair table; the FoNE-specific work is just
encoding + target extraction.
"""
...
from .fone_tokenizer import encode_batch, encode_targets, SEQ_LEN
from .regimes import NUM_REGIMES, REGIME_NAMES


class StratifiedSamplerFone:
    """Equal-mass-per-regime, with-replacement sampler — FoNE-aware."""
    ...
    def __init__(self, table: np.ndarray, train_indices: np.ndarray, seed: int = 0):
        ...
        if len(self.active_regimes) == 0:
            raise ValueError("StratifiedSamplerFone: no training samples in any regime")
...
class EvalBatcherFone:
    """Ordered, exhaustive iterator — FoNE-aware."""
...
def per_regime_eval_loaders_fone(
    ...
    for r in range(NUM_REGIMES):
        ...
        out[r] = EvalBatcherFone(table, indices[mask], batch_size)
    return out
...
def _spot_checks() -> None:
    ...
    sampler = StratifiedSamplerFone(table, train_idx, seed=42)
    batch = sampler.sample_batch(128)
    assert batch["input_ids"].shape == (128, SEQ_LEN - 1)
    assert batch["target_tokens"].shape == (128, SEQ_LEN - 1)
    assert batch["num_values"].shape == (128, SEQ_LEN - 1)
    assert batch["sign_target"].shape == (128,)
    assert batch["digit_target"].shape == (128, 6)
    assert batch["is_nan"].shape == (128,)

    eval_loaders = per_regime_eval_loaders_fone(table, holdout_idx, batch_size=64)
    seen = 0
    for r, loader in eval_loaders.items():
        for b in loader:
            seen += len(b["regime_id"])
            assert (b["regime_id"] == r).all()
    assert seen == len(holdout_idx), f"saw {seen}, expected {len(holdout_idx)}"

    print("all FoNE dataset spot checks passed")
