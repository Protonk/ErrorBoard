"""Launch the FoNE F1 multiplication arm: L4-E048 at 20 seeds.

Mirror of fone_seeds.py with operation="mul". Run names:
`fone-mul-L4-E048-s{N}`.
"""
...
def run_one(seed: int, runs_dir: str, max_iters: int = 20_000) -> None:
    run_name = f"fone-mul-L4-E048-s{seed}"
    run_dir = Path(runs_dir) / run_name
    if _is_completed(run_dir):
        print(f"  {run_name}  SKIP (completed)", flush=True)
        return
    print(f"  === {run_name}  L=4  E=48  seed={seed}  tokenization=fone  operation=mul ===",
          flush=True)
...
def main() -> None:
    ...
    t_start = time.time()
    print(f"\n=== FoNE F1 mult L4-E048 seeds {args.start}..{args.end} ===\n",
          flush=True)
