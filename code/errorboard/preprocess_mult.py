"""Enumerate, classify, and split the 65,536-pair FP8 multiplication table.

Parallel to preprocess.py. Same dtype, same shape, same split semantics — only
the classifier and the regime constants change. Importers should pick which
module to use based on their target operation.
"""
...
TABLE_DTYPE = np.dtype(
    [
        ...
        ("regime_id", np.uint8),
        ("tag_mask", np.uint16),  # 9 tag bits, doesn't fit in uint8
    ]
)
...
def build_table() -> np.ndarray:
    """Enumerate all 65,536 (a_bits, b_bits) pairs and return the classified
...
def split_train_holdout(
    ...
    for r in range(NUM_REGIMES):
        ...
        if n_train < 1:
            raise ValueError(
                f"Regime '{REGIME_NAMES[r]}' has {n} pairs total but holdout "
                f"would consume all of them (n_holdout={n_holdout})."
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build and (optionally) cache the multiplication regime table."
    )
    ...
    print("Building 65,536-pair classified multiplication table...")
    table = build_table()
