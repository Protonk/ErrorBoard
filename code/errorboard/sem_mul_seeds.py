"""Launch the SEM multiplication arm: L4-E048 at 20 seeds.

Mirror of sem_seeds.py with operation="mul". Run names:
`sem-mul-L4-E048-s{N}`.
"""
...
def run_one(seed: int, runs_dir: str, max_iters: int = 20_000) -> None:
    run_name = f"sem-mul-L4-E048-s{seed}"
    run_dir = Path(runs_dir) / run_name
    if _is_completed(run_dir):
        print(f"  {run_name}  SKIP (completed)", flush=True)
        return
    print(f"  === {run_name}  L=4  E=48  seed={seed}  tokenization=sem  operation=mul ===",
          flush=True)
    cfg = TrainingConfig(
        run_name=run_name,
        runs_dir=runs_dir,
...
def main() -> None:
    ...
    t_start = time.time()
    print(f"\n=== SEM mult L4-E048 seeds {args.start}..{args.end} ===\n", flush=True)
