"""Launch the FoNE F2 (binary) reciprocal arm: L4-E048 at 20 seeds.

Mirror of fone_f2_mul_seeds.py with operation="recip". Run names:
`fone-f2-recip-L4-E048-s{N}`.
"""
...
def run_one(seed: int, runs_dir: str, max_iters: int = 20_000) -> None:
    run_name = f"fone-f2-recip-L4-E048-s{seed}"
    run_dir = Path(runs_dir) / run_name
    if _is_completed(run_dir):
        print(f"  {run_name}  SKIP (completed)", flush=True)
        return
    print(f"  === {run_name}  L=4  E=48  seed={seed}  tokenization=fone-f2  operation=recip ===",
          flush=True)
...
def main() -> None:
    ...
    t_start = time.time()
    print(f"\n=== FoNE F2 recip L4-E048 seeds {args.start}..{args.end} ===\n",
          flush=True)
