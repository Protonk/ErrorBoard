"""Training loop for the F2 (binary FoNE) arm.

Mirror of `fone_training.py` but using FoneF2GPT / FoneF2GPTConfig and the
F2 dataset/loss helpers. Same eval streams, checkpoint cadence, and run
metadata format.
"""
...
from .fone_f2_dataset import (
    EvalBatcherFoneF2,
    StratifiedSamplerFoneF2,
    per_regime_eval_loaders_fone_f2,
)
...
def _cosine_lr(iter_num: int, cfg: FoneF2TrainingConfig) -> float:
...
_SIGN_POS = POS_SIGN_C - 1
_DIGIT_POS = POS_NUM_C - 1


@torch.no_grad()
def _eval_pool(model: FoneF2GPT, loader: EvalBatcherFoneF2,
    ...
    for batch in loader:
        ...
        losses = fone_f2_loss(out, target_tokens, sign_target, digit_target, is_nan)
        B = input_ids.shape[0]
        total_loss += float(losses["loss"].item()) * B
        correct = fone_f2_correct(out, sign_target, digit_target, is_nan)
...
def _train_subsample_loaders(table, train_indices, batch_size, n_per_regime):
    ...
    for r in range(NUM_REGIMES):
        ...
        if len(idx) > 0:
            out[r] = EvalBatcherFoneF2(table, idx, batch_size)
    return out
...
def train(cfg: FoneF2TrainingConfig) -> dict:
    ...
    try:
        ...
        sampler = StratifiedSamplerFoneF2(table, train_idx, seed=cfg.seed)
        holdout_eval_loaders = per_regime_eval_loaders_fone_f2(
        ...
        model = FoneF2GPT(f2_cfg).to(device)
        ...
        print(
            ...
            f"tokenization=fone-f2  vocab={VOCAB_SIZE}  seq_len={SEQ_LEN}  "
                    ...
                    torch.save(
                        {
                            ...
                            "fone_f2_gpt_config": dataclasses.asdict(f2_cfg),
                        },
