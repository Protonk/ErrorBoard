"""Launch the bit-level reciprocal arm: L4-E048 at 20 seeds.

Mirror of bit_mul_seeds.py with operation="recip". Run names:
`bit-recip-L4-E048-s{N}`.

For recip we use a larger holdout fraction (20% vs 10%) and lower min_holdout
floor (1 vs 10) because the table has only 256 entries.
"""
...
def run_one(seed: int, runs_dir: str, max_iters: int = 20_000) -> None:
    run_name = f"bit-recip-L4-E048-s{seed}"
    run_dir = Path(runs_dir) / run_name
    if _is_completed(run_dir):
        print(f"  {run_name}  SKIP (completed)", flush=True)
        return
    print(f"  === {run_name}  L=4  E=48  seed={seed}  tokenization=bit  operation=recip ===",
          flush=True)
    cfg = TrainingConfig(
        run_name=run_name,
        runs_dir=runs_dir,
        seed=seed,
        n_layer=4, n_head=4, n_embd=48, d_mlp=192,
...
def main() -> None:
    ...
    t_start = time.time()
    print(f"\n=== bit-level recip L4-E048 seeds {args.start}..{args.end} ===\n",
          flush=True)
